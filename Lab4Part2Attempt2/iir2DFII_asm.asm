...
_iirDFII_asm:
			...
			NOP 	 2
			LDW		.D1	*A1++, A13	; get element from second array. A13 holds DEN2[i]
		    NOP	5
		    LDW		.D1	*A11++, A14 ;A14 holds tempOmega[i]
		    NOP	5
...
LOOP1:
			; A9: i of for loop. A13: den[i],
		       ...
		       NOP	4
		       STW		.D1	A3, *A10[A5] ;Store A10 into tempOmega[index], located at the address stored in A6
		       NOP 	 8
		       STW      .D1 A3, *A11
		       NOP	5
			   ZERO		.L1	A9 ; Zero A9
			   ...
			   MV		.S1		A6, A11		; rest A11 with original pointer


LOOP2:
			   LDW		.D1	*A7++, A13	; get element from second array. A13 holds NUM2[i]
			   NOP	5
		       LDW		.D1	*A11++, A14   ;A14 holds tempOmega[arrayIndex]
		       NOP	5
		       MPYSP	.M1	A14, A13, A15 ; A15 holds the product
		       NOP	8
		       ADDSP	.L1	A3, A15, A3 ; A8 (filterOut) holds (tempOmega[arrayIndex] * NUM2[i])
		       NOP	5
		       ADD 	.L1	A9, 1, A9 ; increment "j"


		       CMPLT 	.L1	A9, B12, A2 	; see if A9 (i) is less than order ;; NEED GREATER THAN OR EQUAL TOO
