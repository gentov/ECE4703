...
interrupt void serialPortRcvISR()
{
    ...
    int rChann = ((int) (temp.channel[0])); // Cast to an int, which can hold 32 bits. So when the 16 bit sample is multiplied by 8 bit coefficient, an effective 24 bit number results.
    ...
    for(i = 0; i < 27; i++)
    {
        ...
        if (arrayIndex < 0) arrayIndex += 27; //wrap around
        filterOut += filter[arrayIndex] * B[i]; //multiply and compute output
    }

    filterOut = filterOut >> 8; // FilterOut has 24 effective bits, so shifting the lower bits out results in a proper 16 bit number
    if (filterOut > 32768)
        DSK6713_LED_on(1);

    rChann = filterOut; //multiply it by 32768 * 0.5 (stop clippin from dc offset)

    temp.channel[0] = rChann; //set the left channel to the new value (s)
